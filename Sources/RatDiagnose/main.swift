...
do {
    ...
    if CommandLine.arguments.contains("--reset-remap") {
        try service.resetRemap(cid: ControlCID.leftButton)
        try service.resetRemap(cid: ControlCID.rightButton)
        print("Reset remap 0x0050/0x0051.")
        for control in controls where control.cid == ControlCID.leftButton || control.cid == ControlCID.rightButton {
            if let flags = try service.reportingFlags(cid: control.cid) {
