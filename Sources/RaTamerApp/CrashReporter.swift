...
private func ratamerSignalHandler(_ sig: Int32) {
...
enum CrashReporter {
    ...
    private static func installSignalHandlers() {
        ...
        sa.__sigaction_u.__sa_handler = ratamerSignalHandler
    ...
    private static func startHangWatchdog() {
        let queue = DispatchQueue(label: "ratamer.hangwatchdog", qos: .utility)
    ...
    private static func writeHeader() {
        ...
        writeAsync("═══ RaTamer \(version) (\(build)) | macOS \(os) | first line of this session ═══\n")
    }
    ...
    private static func warnIfPreviousCrash() {
        ...
        if text.contains("═══ CRASH") {
            print("RaTamer: previous run crashed — see \(logURL.path)")
        }
    }
}
