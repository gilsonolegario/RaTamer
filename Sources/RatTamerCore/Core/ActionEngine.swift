...
public struct ProcessShortcutRunner: ShortcutRunner {
    ...
    public func run(_ name: String) throws {
        ...
        try process.run()
        guard process.waitUntilExit(timeout: 30) else {
...
private extension Process {
    /// Waits for the process to exit, killing it after `timeout` seconds so a
    /// hung osascript/shortcuts call can never freeze the button loop.
...
public final class CGEventPoster: EventPoster {
    ...
    /// Injectable frontmost-app check so the app can avoid dumping synthesized
    /// events into a terminal (where they land as text/commands in tmux).
    public var isFrontmostTerminal: () -> Bool = { false }

    /// Number of synthesized events suppressed because Accessibility is missing.
    public private(set) var blockedPostCount = 0

    /// Number of synthesized events suppressed because a terminal is frontmost.
    public private(set) var terminalBlockedPostCount = 0
    ...
    private func guardNotTerminal() -> Bool {
        guard !isFrontmostTerminal() else {
            terminalBlockedPostCount += 1
            Self.log.info("terminal frontmost — suppressing synthesized event (blocked=\(self.terminalBlockedPostCount, privacy: .public))")
            return false
        }
        return true
    }
    ...
    public func postKey(_ keyCode: UInt16, down: Bool, flags: CGEventFlags) {
        guard guardAccessibility(), guardNotTerminal() else { return }
        if down {
...
public final class ActionEngine {
    ...
    /// Injectable frontmost-app check so system key codes (sent to the
    /// frontmost app via System Events) are not dumped into a terminal.
    public var isFrontmostTerminal: () -> Bool = { false }
    ...
    private func systemKey(_ keyCode: UInt16, _ flags: CGEventFlags) throws {
        guard !isFrontmostTerminal() else {
            Self.log.info("systemKey code=\(keyCode, privacy: .public) suppressed — terminal frontmost")
            return
        }
        var mods: [String] = []
