...
public final class AdjustableDPI {
    ...
    static func expand(_ entries: [UInt16]) -> [UInt16] {
        ...
        while i < entries.count {
            let value = entries[i]
            if value >> 13 == 0b111, i + 1 < entries.count, i > 0 {
            ...
            // Malformed range marker (no previous entry, no end, or step == 0):
            // treat it as a plain value instead of crashing or looping forever.
            result.append(value)
            i += 1
        }
        return result
