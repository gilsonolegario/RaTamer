...
- Gumroad product permalink constant: `"rattamer-pro"` (user creates the product externally with License Keys + PWYW enabled — out of repo scope).
...
final class LicenseClientTests: XCTestCase {
    ...
    func testVerifyPostsFormBodyWithPermalinkAndKey() async throws {
        let captured = expectation(description: "request captured")
        MockURLProtocol.handler = { request in
            let body = request.httpBody.flatMap { String(data: $0, encoding: .utf8) } ?? ""
            XCTAssertEqual(request.httpMethod, "POST")
            XCTAssertTrue(body.contains("product_permalink=rattamer-pro"))
        ...
        let client = GumroadLicenseClient(permalink: "rattamer-pro", session: session)
...
public struct GumroadLicenseClient: LicenseVerifying {
    ...
    public init(permalink: String, session: URLSession = .shared) {
    ...
    public func verify(licenseKey: String) async throws -> LicenseVerification {
        ...
        components.queryItems = [
            URLQueryItem(name: "product_permalink", value: permalink),
...
- Produces: `LicenseService` class with:
  ...
  - `static let productPermalink = "rattamer-pro"` and `static var shared = LicenseService(verifier: GumroadLicenseClient(permalink: productPermalink))`
...
public extension LicenseService {
    static let productPermalink = "rattamer-pro"
    static var shared = LicenseService(
        verifier: GumroadLicenseClient(permalink: productPermalink)
    )
}
```
