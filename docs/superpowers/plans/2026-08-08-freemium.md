...
final class LicenseServiceTests: XCTestCase {
    ...
    private func makeService(verifier: MockVerifier) -> LicenseService {
...
Note: the proGate closure `{ $0 == .runShortcut }` denies everything except `.runShortcut` — that's the point: it proves the gate is feature-specific.
