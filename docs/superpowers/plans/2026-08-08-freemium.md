...
**Interfaces:**
- Consumes: `AppModel.shared.license`, `LicenseService.State`, `LicenseService.productID`, `ProFeature`.
...
> Note: the Gumroad purchase URL slug is the product ID (`rattamer-pro`), which is what `LicenseService.productID` holds — there is no separate `productPermalink` constant (that name belongs to the deprecated verify API field). If the URL slug ever diverges from the product ID, add a dedicated `productPermalink` constant.
...
e) Gate the SmartShift mode actions in `smartShiftMenu` (~lines 236-268) — wrap each `setSmartShiftMode(...)` call. KEEP the `if model.capabilities.hasSmartShift` wrapper around the "SmartShift (auto)" entry (pre-flight): the menu still only offers auto mode when the device supports it; each action is gated by proGate:
